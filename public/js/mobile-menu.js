...
document.addEventListener('DOMContentLoaded', function() {
    // Chỉ tạo và khởi tạo menu mobile khi kích thước màn hình phù hợp
    initMobileMenu();
    
    // Theo dõi thay đổi kích thước màn hình và cập nhật hiển thị menu mobile
    window.addEventListener('resize', function() {
        initMobileMenu();
    });
    
    function initMobileMenu() {
        // Chỉ hiển thị menu mobile trên thiết bị nhỏ hơn 480px
        ...
        // Xử lý icon menu di động
        let mobileMenuIcon = document.querySelector('.mobile-menu-icon');
        
        if (isMobileDevice) {
            // Thêm icon menu nếu chưa có và đang ở thiết bị di động
            if (!mobileMenuIcon) {
                mobileMenuIcon = document.createElement('div');
                ...
                // Thêm sự kiện click cho icon mới tạo
            ...
            // Ẩn icon menu trên thiết bị không phải di động
        ...
        // Chỉ tạo menu nếu chưa có và đang ở thiết bị di động
        if (isMobileDevice && !document.querySelector('.mobile-menu')) {
    ...
    // Hiển thị popup trợ giúp cho lần truy cập đầu tiên
